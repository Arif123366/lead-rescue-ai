  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col selection:bg-cyan-500 selection:text-slate-950">
      ...
      <div className="flex flex-1 min-w-0">
        ...
        <main className="flex-1 min-w-0 p-3 sm:p-6 lg:p-8 space-y-8 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className="glass-card p-4 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-800 hover:border-cyan-500/30 transition-all">
            ...
            <div className="glass-card p-4 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-800 hover:border-purple-500/30 transition-all">
              <div className="flex items-center justify-between text-slate-400 mb-2">
            ...
            <div className="glass-card p-4 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-800 hover:border-indigo-500/30 transition-all">
              <div className="flex items-center justify-between text-slate-400 mb-2">
          ...
          <div className="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-cyan-500/40 rescue-glow space-y-4 shadow-[0_0_25px_rgba(0,240,255,0.12)]">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-cyan-500/20 pb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-purple-500/20 text-purple-400 border border-purple-500/40 flex items-center justify-center neon-purple-glow shrink-0">
              ...
              <Link
                href="/rescue"
                className="text-xs text-cyan-400 hover:text-cyan-300 font-bold flex items-center gap-1 hover:underline py-1 touch-target shrink-0"
              >
            ...
            {needsAttention.length === 0 ? (
              ...
              <div className="space-y-3">
                {needsAttention.map((item: any) => (
                  <div
                    key={item.lead_id}
                    className="p-3.5 sm:p-4 rounded-xl sm:rounded-2xl bg-slate-900/90 border border-slate-800 hover:border-cyan-500/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors"
                  >
                      ...
                      <div className="text-xs text-slate-400 flex items-center gap-4 flex-wrap">
                        ...
                        {item.deal_value > 0 && (
                          <span className="font-mono text-emerald-400 font-bold">
                            Est. Value: ${Number(item.deal_value || 0).toLocaleString()}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0 flex-wrap">
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl rescue-gradient text-slate-950 text-xs font-black flex items-center gap-1.5 hover:opacity-95 transition-opacity disabled:opacity-50 min-h-[38px]"
                      >
                      ...
                      <Link
                        ...
                        className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 min-h-[38px] flex items-center"
                      >
                        Review Lead
                      </Link>
                    </div>
          ...
          <div className="glass-panel p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-800 space-y-4">
            <h3 className="text-xs sm:text-sm font-bold text-white uppercase tracking-wider text-slate-400">
              ...
              {stageBreakdown.map((stg: any) => (
                <div key={stg.name} className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 text-center space-y-1">
                  ...
                  <div className="text-[10px] text-emerald-400 font-mono font-bold truncate">
                    ${Number(stg.value || 0).toLocaleString()}
                  </div>
                </div>
              ))}
            </div>
