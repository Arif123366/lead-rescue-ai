...
export default function LeadDetailPage() {
  ...
  const handleReQualify = async () => {
    setQualifying(true);
    try {
      const res = await fetch(`/api/v1/leads/${leadId}/qualify`, { method: 'POST' });
      if (res.ok) {
        fetchLeadData();
      }
    } catch (err) {
  ...
  const handleSendFollowUp = async () => {
    ...
    try {
      const res = await fetch('/api/v1/followup-messages/send', {
        method: 'POST',
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-4 lg:p-8 space-y-8 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* AI QUALIFICATION ANALYSIS CARD (Left 2 cols) */}
            ...
            {/* APPOINTMENTS & QUICK ACTIONS (Right col) */}
