...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-slate-950">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-16 flex-1 w-full">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-white mb-2 tracking-tight">Get in Touch</h1>
        ...
        {submitted ? (
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6 sm:p-8 text-center">
            <CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto mb-4" />
            <h3 className="text-xl font-bold text-white mb-2">Message Received!</h3>
            <p className="text-xs text-slate-300 max-w-md mx-auto">
              Thank you for reaching out. A team member will respond to your inquiry at <strong className="text-white break-all">{formData.email}</strong> within 4 business hours.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 sm:p-8 space-y-4 sm:space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400 min-h-[44px]"
                />
              </div>
              <div>
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-xl rescue-gradient rescue-glow text-slate-950 text-xs font-black flex items-center justify-center gap-2 hover:opacity-95 transition-opacity min-h-[44px]"
            >
